...
class Admin:
    ...
    async def total(self, ctx):
        ...
        counter = 0
        async for message in client.logs_from(channel=313847718038601729, limit=100):
            if message[:2] == "+$":
                counter += 1
    ...
    async def addrole(self, ctx, user: discord.Member=None):
        ...
        if user is None:
            await self.bot.say("No user selected.")
            return
        
        if since_joined >= 5:
            await self.bot.add_roles(user, TrialEnd)
            await self.bot.say("{}'s trial has ended.".format(user.name))
            await self.bot.say("{} joined {} days ago.".format(user.name, since_joined))

        else:
            await self.bot.say("{} still has trial days.".format(user.name))

        #role = self._role_from_string(server, rolename)
        #TrialEnd = self._role_from_string(server, ("Trial Ended"))

        #if rolename == "Trialist" or rolename == "trialist":
            #await self.bot.say("Testing")
            #await self.bot.add_roles(user, TrialEnd)
            #await self.bot.say('Added role {} to {}'.format(TrialEnd.name, user.name))
            #return
